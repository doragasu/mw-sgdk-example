...
static void udp_reuse_test(void)
{
	...
	// nc -u <dest_ip> 7
	println("Doing echo on UDP port 7", 0);
	// Start UDP echo task
...
static void http_test(void)
{
	...
	println("HTTP test SUCCESS", 0);
	return;

err_out:
	println("HTTP test FAILED", 0);
	return;
}
...
static void tcp_test(void)
{
	...
	println("Connecting to www.example.com", 0);
	err = mw_tcp_connect(1, "www.example.com", "80", NULL);
	if (err != MW_ERR_NONE) {
		println("TCP test FAILED", 0);
		goto out;
	}
	println("DONE!", 0);
	println(NULL, 0);

	println("TCP test SUCCESS", 0);
...
static void datetime_test(void)
{
	...
	do {
		...
		if (!stat) {
			println("Failed to get date/time", 0);
			return;
		}
	} while (!stat->dt_ok);
